...
const thesisSchema = z.object({
  ...
  advisor: z.string().min(2, 'At least one advisor is required'),
...
const EditThesisDialog = ({ thesis, open, onOpenChange }: EditThesisDialogProps) => {
  ...
  const [showExistingPdfPreview, setShowExistingPdfPreview] = useState(false);
  ...
  useEffect(() => {
    if (thesis) {
      ...
      setShowExistingPdfPreview(false);
    }
  }, [thesis, form]);
  ...
  const editThesisMutation = useMutation({
    mutationFn: async (data: ThesisFormData & { file: File | null; thesisId: string }) => {
      ...
      const response = await api.put(`/thesis/${data.thesisId}`, formData, {
        headers: {
    ...
    onError: (error: any) => {
      toast({
        title: 'Error',
        description: error.response?.data?.error || 'Failed to update thesis. Please try again.',
        variant: 'destructive',
      });
    },
  });
  ...
  // Get existing PDF URL (handles both Cloudinary and local URLs)
  const getExistingPdfUrl = () => {
    if (!thesis || !thesis.pdfUrl) return null;
                ...
                <div className="border rounded-lg p-4">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-medium">Existing PDF document</span>
                    </div>
                    <Button
                      type="button"
                      variant="outline"
                      size="sm"
                      onClick={() => setShowExistingPdfPreview(!showExistingPdfPreview)}
                    >
                      <Eye className="h-4 w-4 mr-2" />
                      {showExistingPdfPreview ? 'Hide Preview' : 'Show Preview'}
                    </Button>
                  </div>
                  {showExistingPdfPreview && (
                    <div className="mt-4 overflow-hidden">
                      <iframe
                        src={`${existingPdfUrl}#toolbar=0&navpanes=0&scrollbar=1`}
